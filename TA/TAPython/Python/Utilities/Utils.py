...
# short cut for print dir 打印目录的快捷方式
def d(obj, subString=''):
...
def l(obj, subString='', bPrint=True):
    ...
    for info in editorProperties:
        ...
        if flags[0]:
            ...
            line += "{}{}   {}".format(" " * (formatWidth - len(line) + 2 - 8), resultStr,
                                       descriptionStr)  # descriptionStr 中是[rw]放到最后显示
            strs.append(line)
    ...
    if bPrint:
        ...
        print("{}: matched, builtinCallable: {}  otherCallables: {}  prop: {}  EditorProps: {}  both: {}".format(obj
                                                                                                                 ,
                                                                                                                 len(builtinCallables),
                                                                                                                 len(otherCallables),
                                                                                                                 len(properties),
                                                                                                                 len(editorProperties),
                                                                                                                 propertyAlsoEditorPropertyCount))

    return otherCallables, builtinCallables, properties, editorProperties


# short cut for print type  # 打印类型的快捷方式
def t(obj):
    print(type(obj))


# unreal type to Python dict  # Python dict 的虚幻类型
def ToJson(v):
    tp = type(v)
...
def get_selected_asset():
    ...
    if selected:
        ...
        return unreal.load_asset(selected[0])
    else:
        return None
...
# 创建导出工具以创建帮助/目录到文件
# 通过修改这种映射关系print->stdout,来把我们的打印操作重定向到指定文件
...
def select_actors_with_same_location(actor, error_tolerance):
    if actor is not None:
        actors = select_actors_at_location(actor.get_actor_location(), error_tolerance,
